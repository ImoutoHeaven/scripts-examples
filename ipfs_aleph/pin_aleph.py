...
def pin_cid(cid: str, retries: int) -> bool:
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            # 处理完当前 CID 后立即尝试清理残留 aleph 进程
            kill_aleph_processes()
...
def kill_aleph_processes(timeout: int = 10) -> None:
    """
    跨平台清理所有残留的 aleph 进程。

    Windows  使用 taskkill /F /IM aleph.exe /T
    Unix 系  使用 pkill -f aleph
    """
    is_windows = platform.system() == "Windows"
    try:
        if is_windows:
            cmd = ["taskkill", "/F", "/IM", "aleph.exe", "/T"]
        else:
            cmd = ["pkill", "-f", "aleph"]
        ...
        if result.returncode == 0:
            ...
            print("ℹ️  没有找到需要清理的 aleph 进程")
        else:
            print(f"⚠️  清理命令返回码: {result.returncode}")
            if result.stderr:
                print(f"错误信息: {result.stderr.strip()}")
    except FileNotFoundError:
        print("⚠️  未找到 taskkill/pkill 命令，跳过进程清理")
    except subprocess.TimeoutExpired:
        print("⚠️  清理命令超时")
    except Exception as e:
        print(f"⚠️  清理过程中发生错误: {e}")

def cleanup_aleph_processes():
    """
    兜底清理所有残留的 aleph 进程
    """
    print("\n脚本结束，执行兜底清理 aleph 进程...")
    kill_aleph_processes()
