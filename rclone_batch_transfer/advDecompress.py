...
def ensure_unique_name(target_path, timestamp):
    """Ensure target path is unique by adding timestamp if needed."""
...
class ArchiveProcessor:
    ...
    def process_archive(self, archive_path):
        ...
        # Step 4: Create temporary directory in current working directory with timestamp
        ...
        try:
            ...
            if success:
                ...
                self.apply_decompress_policy(archive_path, tmp_dir, timestamp)
    ...
    def apply_decompress_policy(self, archive_path, tmp_dir, timestamp):
        ...
        if self.args.decompress_policy == 'separate':
            self.apply_separate_policy(tmp_dir, final_output_dir, archive_base_name, timestamp)

        elif self.args.decompress_policy == 'direct':
            self.apply_direct_policy(tmp_dir, final_output_dir, archive_base_name, timestamp)

        else:
            # N-collect policy
            threshold = int(self.args.decompress_policy.split('-')[0])
            self.apply_collect_policy(tmp_dir, final_output_dir, archive_base_name, threshold, timestamp)

    def apply_separate_policy(self, tmp_dir, output_dir, archive_name, timestamp):
        """Apply separate decompress policy following exact specification."""
        separate_dir = f"separate_{timestamp}"
        ...
        try:
            ...
            archive_folder = ensure_unique_name(archive_folder, timestamp)
            ...
            final_archive_path = ensure_unique_name(final_archive_path, timestamp)
    ...
    def apply_direct_policy(self, tmp_dir, output_dir, archive_name, timestamp):
    ...
    def apply_collect_policy(self, tmp_dir, output_dir, archive_name, threshold, timestamp):
        ...
        if total_items >= threshold:
            ...
            self.apply_direct_policy(tmp_dir, output_dir, archive_name, timestamp)
