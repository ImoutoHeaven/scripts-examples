...
def should_skip_archive(archive_path, args, sfx_detector=None):
    ...
    if filename_lower.endswith('.zip'):
        ...
        # 检查传统ZIP策略（新增逻辑）
        if hasattr(args, 'traditional_zip_policy') and args.traditional_zip_policy:
            if is_traditional_zip(archive_path):
                if args.traditional_zip_policy.lower() == 'asis':
                    return True, "传统编码ZIP文件被跳过 (--traditional-zip-policy asis)"
                # 对于其他策略（move, decode-*），不在这里跳过，在process_archive中处理
...
class ArchiveProcessor:
    ...
    def handle_traditional_zip_policy(self, archive_path):
        ...
        # 处理move策略
        if policy == 'move':
            if not self.args.traditional_zip_to:
