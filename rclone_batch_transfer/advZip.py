...
def process_file(file_path, args, base_path):
    ...
    temp_zip_name = f"{temp_name_prefix}.zip"
    ...
    try:
        ...
        if args.out:
            # 使用相对路径作为基础文件名
            output_zip_path = os.path.join(final_output_dir, f"{temp_name_prefix}.zip")
        else:
            output_zip_path = temp_zip_name
        
        # 添加输出路径（使用改进的引用方式）
        z7_cmd.append(quote_path_for_7z(output_zip_path))
        ...
        if args.debug:
            ...
            print(f"- 临时ZIP文件前缀: {temp_name_prefix}")
            print(f"- 输出目录: {final_output_dir}")
            print(f"- 输出ZIP路径: {output_zip_path}")
        ...
        if result.returncode == 0:
            ...
            success, moved_files = find_and_rename_zip_file(
                temp_name_prefix, 
                final_name, 
                final_output_dir, 
                args.debug
...
def process_folder(folder_path, args, base_path):
    ...
    try:
        ...
        # 计算输出文件的完整路径
        if args.out:
            output_zip_path = os.path.join(final_output_dir, temp_zip_name)
        else:
            output_zip_path = os.path.join('..', temp_zip_name)
