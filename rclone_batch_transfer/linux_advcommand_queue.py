...
def format_command_status():
    ...
    with command_lock:
        ...
        # 按状态分组统计
        status_counts = {}
        for state in command_states.values():
            status = state["status"]
            status_counts[status] = status_counts.get(status, 0) + 1
        
        # 显示统计信息
        formatted += "状态统计: "
        status_order = [CommandStatus.SUCCEEDED, CommandStatus.FAILED, CommandStatus.RUNNING, 
                       CommandStatus.PAUSED, CommandStatus.PENDING]
        status_items = []
        for status in status_order:
            if status in status_counts:
                status_items.append(f"{status}: {status_counts[status]}")
        formatted += " | ".join(status_items) + "\n"
        formatted += "-" * 50 + "\n"
        ...
        for cmd_id, state in sorted(command_states.items()):
            ...
            if status == CommandStatus.SUCCEEDED:
                ...
                status_display = f"[‖ {status}]"
            elif status == CommandStatus.PENDING:
                status_display = f"[⋯ {status}]"
            ...
            formatted += f"ID {cmd_id+1:3d}: {status_display:<15} {duration} {retry_info} {command}\n"
        
        formatted += "---------------------\n"
        return formatted

def initialize_command_states(commands):
    """初始化所有命令的状态"""
    with command_lock:
        for i, cmd in enumerate(commands):
            command_states[i] = {
...
def status_updater(interval):
    ...
    while not stop_event.is_set():
        ...
        if not stop_event.is_set():  # 睡眠后再次检查
            print(format_command_status())
            print("\n提示: 输入 'status' 查看状态, 'pause/resume' 暂停/恢复命令, 'exit' 退出\n")

def execute_command(cmd_id, cmd, total_retries):
    """执行命令，支持重试和暂停/恢复"""
    # 命令状态已在主函数中初始化，这里只需要更新
...
def main():
    try:
        ...
        # 初始化所有命令状态
        initialize_command_states(commands)
