...
def main(folder_path, dry_run=False, debug=False):
    """
    主函数：遍历folder_path里的所有文件，进行改名。
    如遇同名冲突，比较文件修改时间和大小，较旧或较小者移动到temp。
    在改名完成后，如果 is_filename_compliant => reorder_suffix 做二次后缀排。
    """
    warnings = []
    temp_dir = None
    total_files = 0
        ...
        for item in files:
            ...
            try:
                ...
                if item_path != new_path:
                    if dry_run:
                        print(f"\n[DRY RUN] Would rename: {item_path} -> {new_path}")
                    else:
                        try:
                            os.rename(item_path, new_path)
                            if debug:
                                print(f"\n[DEBUG] Renamed: {item_path} -> {new_path}")
                        except OSError as e:
                    ...
                    if reordered != base_no_ext:
                        ...
                        if not dry_run and final_path != new_path:
                            try:
                                ...
                                if debug:
                                    print(f"\n[DEBUG] Reordered suffix: {new_path} -> {final_path}")
                            except OSError as e:
                                if hasattr(e, 'winerror') and e.winerror == 183:
                                    if temp_dir is None:
                                        temp_dir = ensure_temp_dir(folder_path, debug=debug)
                            ...
                            if dry_run and final_path != new_path:
                                print(f"\n[DRY RUN] Would reorder suffix: {new_path} -> {final_path}")
                else:
                    # 不合规则 => 警告
